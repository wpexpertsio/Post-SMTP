...
use PostSMTP\Vendor\Monolog\Handler\HandlerInterface;
use PostSMTP\Vendor\Monolog\Handler\StreamHandler;
use PostSMTP\Vendor\Psr\Log\LoggerInterface;
use PostSMTP\Vendor\Psr\Log\InvalidArgumentException;
use Exception;
/**
 * Monolog log channel
 *
 ...
 */
class Logger implements \PostSMTP\Vendor\Psr\Log\LoggerInterface, \PostSMTP\Vendor\Monolog\ResettableInterface
{
    /**
     ...
     * @return $this
     */
    public function pushHandler(\PostSMTP\Vendor\Monolog\Handler\HandlerInterface $handler)
    {
    ...
    {
        if (!$this->handlers) {
            $this->pushHandler(new \PostSMTP\Vendor\Monolog\Handler\StreamHandler('php://stderr', static::DEBUG));
        }
    ...
    public static function getLevelName($level)
    {
        if (!isset(static::$levels[$level])) {
            throw new \PostSMTP\Vendor\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
