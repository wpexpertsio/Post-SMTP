	...
	class PostmanEmailLogService {
		...
		private function writeToEmailLog( PostmanEmailLog $log, PostmanMessage $message = null ) {
			...
			if( $this->new_logging ) {
				...
				$data['original_to'] = !empty( $log->originalTo ) && is_string( $log->originalTo ) ? implode( ',', array_map( 'sanitize_email', explode( ',', $log->originalTo ) ) ) : '';
				$data['original_subject'] = !empty( $log->originalSubject ) ? sanitize_text_field( $log->originalSubject ) : '';
				$data['original_message'] = !empty( $log->originalMessage ) ? wp_kses_post( $log->originalMessage ) : '';
