	...
	class PostmanState {
		...
		public function isTimeToReviewPostman() {
			if ( ! empty( $this->options [ self::INSTALL_DATE ] ) ) {
				$successful = self::getInstance()->getSuccessfulDeliveries() > 0;
		...
		function getSuccessfulDeliveries() {
			if ( isset( $this->options [ self::DELIVERY_SUCCESS_TOTAL ] ) ) {
				return $this->options [ self::DELIVERY_SUCCESS_TOTAL ];
			} else {
				return 0;
			}
		}
		function setSuccessfulDelivery( $total ) {
			$this->options [ self::DELIVERY_SUCCESS_TOTAL ] = $total;
		}
		...
		function getFailedDeliveries() {
			if ( isset( $this->options [ self::DELIVERY_FAILURE_TOTAL ] ) ) {
				return $this->options [ self::DELIVERY_FAILURE_TOTAL ];
			} else {
				return 0;
			}
		}
		function setFailedDelivery( $total ) {
			$this->options [ self::DELIVERY_FAILURE_TOTAL ] = $total;
		}
