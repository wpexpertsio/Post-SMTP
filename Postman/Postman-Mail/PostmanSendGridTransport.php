...
class PostmanSendGridTransport extends PostmanAbstractModuleTransport implements PostmanModuleTransport {
	...
	public function createMailEngine() {
		$api_key = Postman_Connection_Resolver::get_primary_field(
			'sendgrid_api_key',
			array( $this->options, 'getSendGridApiKey' )
		);

		require_once 'PostmanSendGridMailEngine.php';
		return new PostmanSendGridMailEngine( $api_key );
	}
	/**
	 * @since 3.0.1
	 * @version 1.0
	...
	public function createMailEngineFallback() {
		...
		$api_credentials = array(
			'api_key'     => Postman_Connection_Resolver::get_fallback_field( 'sendgrid_api_key' ),
			'is_fallback' => 1,
		);
		require_once 'PostmanSendGridMailEngine.php';
		return new PostmanSendGridMailEngine( $api_credentials );
	}
