...
class PostmanMandrillTransport extends PostmanAbstractModuleTransport implements PostmanModuleTransport {
	...
	public function createMailEngine() {
		$apiKey = Postman_Connection_Resolver::get_primary_field(
			'mandrill_api_key',
			array( $this->options, 'getMandrillApiKey' )
		);
	...
	public function createMailEngineFallback() {
		$api_credentials = array(
			'api_key'     => Postman_Connection_Resolver::get_fallback_field( 'mandrill_api_key' ),
			'is_fallback' => 1,
		);
		require_once 'PostmanMandrillMailEngine.php';
		return new PostmanMandrillMailEngine( $api_credentials );
	}
	...
	public function addSettings() {
		...
		add_settings_section(
			self::MANDRILL_AUTH_SECTION,
			__( 'Authentication', 'post-smtp' ),
			array(
				$this,
				'printMandrillAuthSectionInfo',
			),
			self::MANDRILL_AUTH_OPTIONS
		);

		add_settings_field(
			PostmanOptions::MANDRILL_API_KEY,
	...
	public function registerStylesAndScripts() {
		...
		wp_register_script(
			'postman_mandrill_script',
			plugins_url( 'Postman/Postman-Mail/postman_mandrill.js', $this->rootPluginFilenameAndPath ),
			array(
