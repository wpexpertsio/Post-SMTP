...
if ( ! class_exists( 'PostmanMyMailConnector' ) ) {
	class PostmanMyMailConnector {
		...
		public function activate() {
			if ( defined( 'MAILSTER_VERSION' ) && version_compare( MAILSTER_POSTMAN_REQUIRED_VERSION, MAILSTER_VERSION, '<=' ) ) {
				mailster_notice( sprintf( __( 'Mailster: Change the delivery method in the %s!', 'post-smtp' ), sprintf( '<a href="edit.php?post_type=newsletter&page=mailster_settings&mailster_remove_notice=mailster_delivery_method#delivery">%s</a>', __( 'Settings', 'post-smtp' ) ) ), '', false, 'delivery_method' );
				$this->reset();
			}
		}
