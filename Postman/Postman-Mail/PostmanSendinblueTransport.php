	...
	class PostmanSendinblueTransport extends PostmanAbstractModuleTransport implements PostmanModuleTransport {
		...
		public function createMailEngine() {
			$api_key = Postman_Connection_Resolver::get_primary_field(
				'sendinblue_api_key',
				array( $this->options, 'getSendinblueApiKey' )
			);

			require_once 'PostmanSendinblueMailEngine.php';
			return new PostmanSendinblueMailEngine( $api_key );
		}
		...
		public function createMailEngineFallback() {
			$api_credentials = array(
				'api_key'     => Postman_Connection_Resolver::get_fallback_field( 'sendinblue_api_key' ),
				'is_fallback' => 1,
			);
			require_once 'PostmanSendinblueMailEngine.php';
			return new PostmanSendinblueMailEngine( $api_credentials );
		}
