...
final class FnStream implements \PostSMTP\Vendor\Psr\Http\Message\StreamInterface {

	private const SLOTS = array( '__toString', 'close', 'detach', 'rewind', 'getSize', 'tell', 'eof', 'isSeekable', 'seek', 'isWritable', 'write', 'isReadable', 'read', 'getContents', 'getMetadata' );
	 ...
	 * @return FnStream
	 */
	public static function decorate( \PostSMTP\Vendor\Psr\Http\Message\StreamInterface $stream, array $methods ) {
		// If any of the required methods were not provided, then simply
		// proxy to the decorated stream.
		foreach ( \array_diff( self::SLOTS, \array_keys( $methods ) ) as $diff ) {
			/** @var callable $callable */
			$callable         = array( $stream, $diff );
