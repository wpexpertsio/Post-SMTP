...
final class UriNormalizer
{
     ...
     */
    public static function normalize(\PostSMTP\Vendor\Psr\Http\Message\UriInterface $uri, int $flags = self::PRESERVING_NORMALIZATIONS) : \PostSMTP\Vendor\Psr\Http\Message\UriInterface
    {
        ...
        if ($flags & self::REMOVE_DEFAULT_PORT && $uri->getPort() !== null && \PostSMTP\Vendor\GuzzleHttp\Psr7\Uri::isDefaultPort($uri)) {
            $uri = $uri->withPort(null);
        }
        if ($flags & self::REMOVE_DOT_SEGMENTS && !\PostSMTP\Vendor\GuzzleHttp\Psr7\Uri::isRelativePathReference($uri)) {
            $uri = $uri->withPath(\PostSMTP\Vendor\GuzzleHttp\Psr7\UriResolver::removeDotSegments($uri->getPath()));
        }
     ...
     */
    public static function isEquivalent(\PostSMTP\Vendor\Psr\Http\Message\UriInterface $uri1, \PostSMTP\Vendor\Psr\Http\Message\UriInterface $uri2, int $normalizations = self::PRESERVING_NORMALIZATIONS) : bool
    {
        return (string) self::normalize($uri1, $normalizations) === (string) self::normalize($uri2, $normalizations);
    }
    private static function capitalizePercentEncoding(\PostSMTP\Vendor\Psr\Http\Message\UriInterface $uri) : \PostSMTP\Vendor\Psr\Http\Message\UriInterface
    {
        $regex = '/(?:%[A-Fa-f0-9]{2})++/';
    ...
    private static function decodeUnreservedCharacters(\PostSMTP\Vendor\Psr\Http\Message\UriInterface $uri) : \PostSMTP\Vendor\Psr\Http\Message\UriInterface
    {
