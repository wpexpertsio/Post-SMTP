...
use PostSMTP\Vendor\PhpAmqpLib\Message\AMQPMessage;
use PostSMTP\Vendor\PhpAmqpLib\Channel\AMQPChannel;
use AMQPExchange;
class AmqpHandler extends \PostSMTP\Vendor\Monolog\Handler\AbstractProcessingHandler
{
    /**
     ...
     */
    public function __construct($exchange, $exchangeName = 'log', $level = \PostSMTP\Vendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if ($exchange instanceof \AMQPExchange) {
            $exchange->setName($exchangeName);
        } elseif ($exchange instanceof \PostSMTP\Vendor\PhpAmqpLib\Channel\AMQPChannel) {
    ...
    {
        return new \PostSMTP\Vendor\PhpAmqpLib\Message\AMQPMessage((string) $data, array('delivery_mode' => 2, 'content_type' => 'application/json'));
    }
    /**
     * {@inheritDoc}
     */
