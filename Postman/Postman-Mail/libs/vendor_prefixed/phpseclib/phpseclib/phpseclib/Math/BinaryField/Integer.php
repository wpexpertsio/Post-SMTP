 ...
 */
namespace PostSMTP\Vendor\phpseclib3\Math\BinaryField;
...
class Integer extends \PostSMTP\Vendor\phpseclib3\Math\Common\FiniteField\Integer {
	...
	private static function polynomialDivide( $x, $y ) {
		...
		while ( ( $degr = static::deg( $r ) ) >= $d ) {
			...
			$s      = \PostSMTP\Vendor\phpseclib3\Math\BinaryField::base2ToBase256( $s );
		...
		return array( \ltrim( $q, "\0" ), \ltrim( $r, "\0" ) );
	}
	/**
	...
	private static function regularPolynomialMultiply( $x, $y ) {
		$precomputed = array( \ltrim( $x, "\0" ) );
		$x           = \strrev( \PostSMTP\Vendor\phpseclib3\Math\BinaryField::base256ToBase2( $x ) );
		$y           = \strrev( \PostSMTP\Vendor\phpseclib3\Math\BinaryField::base256ToBase2( $y ) );
		...
		$result = \PostSMTP\Vendor\phpseclib3\Math\BinaryField::base2ToBase256( $result );
		$size   = \strlen( $result );
		$x      = \strrev( $x );
		// precompute left shift 1 through 7
		for ( $i = 1; $i < 8; $i++ ) {
			$precomputed[ $i ] = \PostSMTP\Vendor\phpseclib3\Math\BinaryField::base2ToBase256( $x . \str_repeat( '0', $i ) );
		}
	...
	public function toBits() {
		...
		return \PostSMTP\Vendor\phpseclib3\Math\BinaryField::base256ToBase2( $this->value );
	}
	/**
	...
	public function toBigInteger() {
		return new \PostSMTP\Vendor\phpseclib3\Math\BigInteger( $this->value, 256 );
	}
	/**
