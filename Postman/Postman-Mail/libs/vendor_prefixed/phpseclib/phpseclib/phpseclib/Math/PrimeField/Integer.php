 ...
 */
namespace PostSMTP\Vendor\phpseclib3\Math\PrimeField;

use PostSMTP\Vendor\phpseclib3\Common\Functions\Strings;
use PostSMTP\Vendor\phpseclib3\Math\BigInteger;
use PostSMTP\Vendor\phpseclib3\Math\Common\FiniteField\Integer as Base;
/**
 * Prime Finite Fields
 *
 * @author  Jim Wigginton <terrafrost@php.net>
 */
class Integer extends \PostSMTP\Vendor\phpseclib3\Math\Common\FiniteField\Integer {
	 ...
	 */
	public function __construct( $instanceID, \PostSMTP\Vendor\phpseclib3\Math\BigInteger $num = null ) {
	 ...
	 * @return void
	 */
	public static function setModulo( $instanceID, \PostSMTP\Vendor\phpseclib3\Math\BigInteger $modulo ) {
	...
	public static function setRecurringModuloFunction( $instanceID, callable $function ) {
		...
		if ( ! isset( static::$zero[ static::class ] ) ) {
			static::$zero[ static::class ] = new \PostSMTP\Vendor\phpseclib3\Math\BigInteger();
		}
	}
	/**
	 ...
	 * @return static
	 */
	public function pow( \PostSMTP\Vendor\phpseclib3\Math\BigInteger $x ) {
		$temp        = new static( $this->instanceID );
	...
	public function squareRoot() {
		...
		if ( ! isset( $one ) ) {
			...
			$two = new \PostSMTP\Vendor\phpseclib3\Math\BigInteger( 2 );
		}
		$reduce    = static::$reduce[ $this->instanceID ];
		$p_1       = static::$modulo[ $this->instanceID ]->subtract( $one );
		$q         = clone $p_1;
		$s         = \PostSMTP\Vendor\phpseclib3\Math\BigInteger::scan1divide( $q );
		...
		$m          = new \PostSMTP\Vendor\phpseclib3\Math\BigInteger( $s );
	...
	public function toHex() {
		return \PostSMTP\Vendor\phpseclib3\Common\Functions\Strings::bin2hex( $this->toBytes() );
	}
	/**
	...
	public function getNAF( $w = 1 ) {
		++$w;
		$mask = new \PostSMTP\Vendor\phpseclib3\Math\BigInteger( ( 1 << $w ) - 1 );
		$sub  = new \PostSMTP\Vendor\phpseclib3\Math\BigInteger( 1 << $w );
