 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
abstract class MontgomeryMult extends \PostSMTP\Vendor\phpseclib3\Math\BigInteger\Engines\PHP\Reductions\Montgomery {
	...
	public static function multiplyReduce( array $x, array $y, array $m, $class ) {
		...
		$y = \array_pad( $y, $n, 0 );
		$m = \array_pad( $m, $n, 0 );
		$a = array( self::VALUE => self::array_repeat( 0, $n + 1 ) );
		for ( $i = 0; $i < $n; ++$i ) {
			...
			$temp             = $temp - $class::BASE_FULL * ( $class::BASE === 26 ? \intval( $temp / 0x4000000 ) : $temp >> 31 );
			$temp             = $class::addHelper( $class::regularMultiply( array( $x[ $i ] ), $y ), \false, $class::regularMultiply( array( $temp ), $m ), \false );
