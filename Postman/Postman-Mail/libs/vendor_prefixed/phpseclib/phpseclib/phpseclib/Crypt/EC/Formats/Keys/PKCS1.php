...
abstract class PKCS1 extends \PostSMTP\Vendor\phpseclib3\Crypt\Common\Formats\Keys\PKCS1 {
	...
	public static function load( $key, $password = '' ) {
		...
		if ( \strpos( $key, 'BEGIN EC PARAMETERS' ) && \strpos( $key, 'BEGIN EC PRIVATE KEY' ) ) {
			...
			$decoded = \PostSMTP\Vendor\phpseclib3\File\ASN1::decodeBER( $decoded );
			if ( ! $decoded ) {
				throw new \RuntimeException( 'Unable to decode BER' );
			}
			$ecPrivate = \PostSMTP\Vendor\phpseclib3\File\ASN1::asn1map( $decoded[0], \PostSMTP\Vendor\phpseclib3\File\ASN1\Maps\ECPrivateKey::MAP );
			...
			$ecParams = \PostSMTP\Vendor\phpseclib3\File\ASN1::asn1map( $decoded[0], \PostSMTP\Vendor\phpseclib3\File\ASN1\Maps\ECParameters::MAP );
			...
			if ( isset( $components['curve'] ) && self::encodeParameters( $ecParams, \false, array() ) != self::encodeParameters( $components['curve'], \false, array() ) ) {
			...
			$components['dA'] = new \PostSMTP\Vendor\phpseclib3\Math\BigInteger( $ecPrivate['privateKey'], 256 );
		...
		$key = \PostSMTP\Vendor\phpseclib3\File\ASN1::asn1map( $decoded[0], \PostSMTP\Vendor\phpseclib3\File\ASN1\Maps\ECParameters::MAP );
		if ( \is_array( $key ) ) {
			return array( 'curve' => self::loadCurveByParam( $key ) );
		}
	 ...
	 * @return string
	 */
	public static function saveParameters( \PostSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve, array $options = array() ) {
		self::initialize_static_variables();
	...
	public static function savePrivateKey( \PostSMTP\Vendor\phpseclib3\Math\BigInteger $privateKey, \PostSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve, array $publicKey, $secret = null, $password = '', array $options = array() ) {
		self::initialize_static_variables();
		if ( $curve instanceof \PostSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards || $curve instanceof \PostSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Montgomery ) {
			throw new \PostSMTP\Vendor\phpseclib3\Exception\UnsupportedCurveException( 'TwistedEdwards Curves are not supported' );
		}
		$publicKey = "\4" . $publicKey[0]->toBytes() . $publicKey[1]->toBytes();
		$key       = array(
			'version'    => 'ecPrivkeyVer1',
			'privateKey' => $privateKey->toBytes(),
			'parameters' => new \PostSMTP\Vendor\phpseclib3\File\ASN1\Element( self::encodeParameters( $curve ) ),
