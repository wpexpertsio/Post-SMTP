...
abstract class OpenSSH extends \PostSMTP\Vendor\phpseclib3\Crypt\Common\Formats\Keys\OpenSSH {
	 ...
	 * @var array
	 */
	protected static $types = array( 'ecdsa-sha2-nistp256', 'ecdsa-sha2-nistp384', 'ecdsa-sha2-nistp521', 'ssh-ed25519' );
	/**
	...
	public static function load( $key, $password = '' ) {
		...
		if ( isset( $parsed['paddedKey'] ) ) {
			...
			$curve = self::loadCurveByParam( array( 'namedCurve' => $curveName ) );
	...
	public static function savePrivateKey( \PostSMTP\Vendor\phpseclib3\Math\BigInteger $privateKey, \PostSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve, array $publicKey, $secret = null, $password = '', array $options = array() ) {
		...
		$alias      = self::getAlias( $curve );
		$points     = "\4" . $publicKey[0]->toBytes() . $publicKey[1]->toBytes();
		$publicKey  = self::savePublicKey( $curve, $publicKey, array( 'binary' => \true ) );
