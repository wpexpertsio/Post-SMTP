...
abstract class OpenSSH extends \PostSMTP\Vendor\phpseclib3\Crypt\Common\Formats\Keys\OpenSSH {
	 ...
	 * @var array
	 */
	protected static $types = array( 'ssh-rsa' );
	/**
	...
	public static function load( $key, $password = '' ) {
		...
		if ( isset( $parsed['paddedKey'] ) ) {
			...
			$primes = $coefficients = array();
			list($modulus, $publicExponent, $privateExponent, $coefficients[2], $primes[1], $primes[2], $comment, ) = \PostSMTP\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2( 'i6s', $parsed['paddedKey'] );
			$temp        = $primes[1]->subtract( $one );
			$exponents   = array( 1 => $publicExponent->modInverse( $temp ) );
		...
		list($publicExponent, $modulus) = \PostSMTP\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2( 'ii', $parsed['publicKey'] );
		return array(
			'isPublicKey'    => \true,
			'modulus'        => $modulus,
	...
	public static function savePublicKey( \PostSMTP\Vendor\phpseclib3\Math\BigInteger $n, \PostSMTP\Vendor\phpseclib3\Math\BigInteger $e, array $options = array() ) {
		$RSAPublicKey = \PostSMTP\Vendor\phpseclib3\Common\Functions\Strings::packSSH2( 'sii', 'ssh-rsa', $e, $n );
	...
	public static function savePrivateKey( \PostSMTP\Vendor\phpseclib3\Math\BigInteger $n, \PostSMTP\Vendor\phpseclib3\Math\BigInteger $e, \PostSMTP\Vendor\phpseclib3\Math\BigInteger $d, array $primes, array $exponents, array $coefficients, $password = '', array $options = array() ) {
		$publicKey  = self::savePublicKey( $n, $e, array( 'binary' => \true ) );
		$privateKey = \PostSMTP\Vendor\phpseclib3\Common\Functions\Strings::packSSH2( 'si6', 'ssh-rsa', $n, $e, $d, $coefficients[2], $primes[1], $primes[2] );
