...
abstract class PKCS8 extends \PostSMTP\Vendor\phpseclib3\Crypt\Common\Formats\Keys\PKCS8 {
	 ...
	 * @return string
	 */
	public static function savePrivateKey( \PostSMTP\Vendor\phpseclib3\Math\BigInteger $prime, \PostSMTP\Vendor\phpseclib3\Math\BigInteger $base, \PostSMTP\Vendor\phpseclib3\Math\BigInteger $privateKey, \PostSMTP\Vendor\phpseclib3\Math\BigInteger $publicKey, $password = '', array $options = array() ) {
		$params = array(
			'prime' => $prime,
			'base'  => $base,
		);
		...
		$key    = \PostSMTP\Vendor\phpseclib3\File\ASN1::encodeDER( $privateKey, array( 'type' => \PostSMTP\Vendor\phpseclib3\File\ASN1::TYPE_INTEGER ) );
	 ...
	 * @return string
	 */
	public static function savePublicKey( \PostSMTP\Vendor\phpseclib3\Math\BigInteger $prime, \PostSMTP\Vendor\phpseclib3\Math\BigInteger $base, \PostSMTP\Vendor\phpseclib3\Math\BigInteger $publicKey, array $options = array() ) {
		$params = array(
			'prime' => $prime,
			'base'  => $base,
		);
		$params = \PostSMTP\Vendor\phpseclib3\File\ASN1::encodeDER( $params, \PostSMTP\Vendor\phpseclib3\File\ASN1\Maps\DHParameter::MAP );
		$params = new \PostSMTP\Vendor\phpseclib3\File\ASN1\Element( $params );
		$key    = \PostSMTP\Vendor\phpseclib3\File\ASN1::encodeDER( $publicKey, array( 'type' => \PostSMTP\Vendor\phpseclib3\File\ASN1::TYPE_INTEGER ) );
