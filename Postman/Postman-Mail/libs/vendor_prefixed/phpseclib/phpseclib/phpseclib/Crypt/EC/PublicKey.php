 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
class PublicKey extends \PostSMTP\Vendor\phpseclib3\Crypt\EC implements \PostSMTP\Vendor\phpseclib3\Crypt\Common\PublicKey {
	...
	public function verify( $message, $signature ) {
		...
		if ( $this->curve instanceof \PostSMTP\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards ) {
			if ( $shortFormat == 'SSH2' ) {
				list(, $signature) = \PostSMTP\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2( 'ss', $signature );
			}
			...
			try {
				$R = \PostSMTP\Vendor\phpseclib3\Crypt\EC\Formats\Keys\PKCS1::extractPoint( $R, $curve );
				$R = $this->curve->convertToInternal( $R );
			} catch ( \Exception $e ) {
				return \false;
			}
			$S = \strrev( $S );
			$S = new \PostSMTP\Vendor\phpseclib3\Math\BigInteger( $S, 256 );
		...
		if ( self::$engines['OpenSSL'] && \in_array( $this->hash->getHash(), \openssl_get_md_methods() ) ) {
			$sig    = $format != 'ASN1' ? \PostSMTP\Vendor\phpseclib3\Crypt\EC\Formats\Signature\ASN1::save( $r, $s ) : $signature;
			$result = \openssl_verify( $message, $sig, $this->toString( 'PKCS8', array( 'namedCurve' => \false ) ), $this->hash->getHash() );
			if ( $result != -1 ) {
				return (bool) $result;
			}
		}
		...
		list($x1, $y1) = $this->curve->multiplyAddPoints( array( $this->curve->getBasePoint(), $this->QA ), array( $u1, $u2 ) );
		$x1            = $x1->toBigInteger();
