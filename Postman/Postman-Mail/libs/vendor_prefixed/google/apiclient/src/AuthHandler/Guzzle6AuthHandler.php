...
class Guzzle6AuthHandler {
	...
	public function __construct( \PostSMTP\Vendor\Psr\Cache\CacheItemPoolInterface $cache = null, array $cacheConfig = array() ) {
		$this->cache       = $cache;
	...
	public function attachKey( \PostSMTP\Vendor\GuzzleHttp\ClientInterface $http, $key ) {
		$middleware = new \PostSMTP\Vendor\Google\Auth\Middleware\SimpleMiddleware( array( 'key' => $key ) );
		$config     = $http->getConfig();
		$config['handler']->remove( 'google_auth' );
		$config['handler']->push( $middleware, 'google_auth' );
		$config['auth'] = 'simple';
	...
	private function createAuthHttp( \PostSMTP\Vendor\GuzzleHttp\ClientInterface $http ) {
		return new \PostSMTP\Vendor\GuzzleHttp\Client( array( 'http_errors' => \true ) + $http->getConfig() );
	}
}
