...
class Client
{
    ...
    public function __construct(array $config = array())
    {
        ...
        if (!\is_null($this->config['credentials'])) {
            if ($this->config['credentials'] instanceof \PostSMTP\Vendor\Google\Auth\CredentialsLoader) {
    ...
    public function fetchAccessTokenWithAuthCode($code)
    {
        ...
        $httpHandler = \PostSMTP\Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build($this->getHttpClient());
     ...
     * @return array access token
     */
    public function fetchAccessTokenWithAssertion(\PostSMTP\Vendor\GuzzleHttp\ClientInterface $authHttp = null)
    {
        ...
        $httpHandler = \PostSMTP\Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build($authHttp);
     ...
     */
    public function authorize(\PostSMTP\Vendor\GuzzleHttp\ClientInterface $http = null)
    {
    ...
    {
        $tokenRevoker = new \PostSMTP\Vendor\Google\AccessToken\Revoke($this->getHttpClient());
    ...
    {
        $tokenVerifier = new \PostSMTP\Vendor\Google\AccessToken\Verify($this->getHttpClient(), $this->getCache(), $this->config['jwt']);
        if (null === $idToken) {
    ...
    public function execute(\PostSMTP\Vendor\Psr\Http\Message\RequestInterface $request, $expectedClass = null)
    {
        ...
        $http = $this->authorize();
        return \PostSMTP\Vendor\Google\Http\REST::execute($http, $request, $expectedClass, $this->config['retry'], $this->config['retry_map']);
    }
    /**
    ...
    {
        $auth = new \PostSMTP\Vendor\Google\Auth\OAuth2(['clientId' => $this->getClientId(), 'clientSecret' => $this->getClientSecret(), 'authorizationUri' => self::OAUTH2_AUTH_URL, 'tokenCredentialUri' => self::OAUTH2_TOKEN_URI, 'redirectUri' => $this->getRedirectUri(), 'issuer' => $this->config['client_id'], 'signingKey' => $this->config['signing_key'], 'signingAlgorithm' => $this->config['signing_algorithm']]);
        return $auth;
    }
    /**
     ...
     */
    public function setCache(\PostSMTP\Vendor\Psr\Cache\CacheItemPoolInterface $cache)
    {
        $this->cache = $cache;
    }
    /**
     ...
     */
    public function setLogger(\PostSMTP\Vendor\Psr\Log\LoggerInterface $logger)
    {
        $this->logger = $logger;
    }
    /**
    ...
    {
        $logger = new \PostSMTP\Vendor\Monolog\Logger('google-api-php-client');
        if ($this->isAppEngine()) {
            $handler = new \PostSMTP\Vendor\Monolog\Handler\SyslogHandler('app', \LOG_USER, \PostSMTP\Vendor\Monolog\Logger::NOTICE);
        } else {
            $handler = new \PostSMTP\Vendor\Monolog\Handler\StreamHandler('php://stderr', \PostSMTP\Vendor\Monolog\Logger::NOTICE);
        }
    ...
    {
        return new \PostSMTP\Vendor\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
     * Set the Http Client object
     * @param ClientInterface $http
     */
    public function setHttpClient(\PostSMTP\Vendor\GuzzleHttp\ClientInterface $http)
    {
        $this->http = $http;
    }
    /**
    ...
    protected function createDefaultHttpClient()
    {
        ...
        return new \PostSMTP\Vendor\GuzzleHttp\Client($options);
    }
    /**
    ...
    private function createApplicationDefaultCredentials()
    {
        ...
        if ($signingKey) {
            ...
            $credentials = \PostSMTP\Vendor\Google\Auth\ApplicationDefaultCredentials::getCredentials($scopes, null, $sub ? null : $this->config['cache_config'], $sub ? null : $this->getCache(), $this->config['quota_project']);
        }
        // for service account domain-wide authority (impersonating a user)
        // @see https://developers.google.com/identity/protocols/OAuth2ServiceAccount
        if ($sub) {
            if (!$credentials instanceof \PostSMTP\Vendor\Google\Auth\Credentials\ServiceAccountCredentials) {
        ...
        if (!$credentials instanceof \PostSMTP\Vendor\Google\Auth\FetchAuthTokenCache) {
            $credentials = new \PostSMTP\Vendor\Google\Auth\FetchAuthTokenCache($credentials, $this->config['cache_config'], $this->getCache());
        }
        return $credentials;
    }
    protected function getAuthHandler()
    {
        ...
        return \PostSMTP\Vendor\Google\AuthHandler\AuthHandlerFactory::build($this->getCache(), $this->config['cache_config']);
    }
