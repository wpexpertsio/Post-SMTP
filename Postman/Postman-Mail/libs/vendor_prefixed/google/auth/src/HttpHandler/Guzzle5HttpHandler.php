...
class Guzzle5HttpHandler {
	...
	public function async( \PostSMTP\Vendor\Psr\Http\Message\RequestInterface $request, array $options = array() ) {
		...
		$futureResponse = $this->client->send( $this->createGuzzle5Request( $request, array( 'future' => \true ) + $options ) );
		...
		return $promise->then(
			function ( \PostSMTP\Vendor\GuzzleHttp\Message\ResponseInterface $response ) {
	...
	private function createPsr7Response( \PostSMTP\Vendor\GuzzleHttp\Message\ResponseInterface $response ) {
		return new \PostSMTP\Vendor\GuzzleHttp\Psr7\Response( $response->getStatusCode(), $response->getHeaders() ?: array(), $response->getBody(), $response->getProtocolVersion(), $response->getReasonPhrase() );
	}
}
