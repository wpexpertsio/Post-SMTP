...
use PostSMTP\Vendor\phpseclib\Math\BigInteger as BigInteger2;
use PostSMTP\Vendor\phpseclib3\Crypt\PublicKeyLoader;
use PostSMTP\Vendor\phpseclib3\Math\BigInteger as BigInteger3;
...
class AccessToken {
	...
	public function verify( $token, array $options = array() ) {
		...
		if ( ! \in_array( $alg, array( 'RS256', 'ES256' ) ) ) {
	...
	private function verifyEs256( $token, array $certs, $audience = null, $issuer = null ) {
		...
		$jwt     = $this->callSimpleJwtDecode( array( $token, $jwkset, 'ES256' ) );
	...
	private function verifyRs256( $token, array $certs, $audience = null, $issuer = null ) {
		...
		foreach ( $certs as $cert ) {
			...
			$keys[ $cert['kid'] ] = new \PostSMTP\Vendor\Firebase\JWT\Key( $publicKey, 'RS256' );
		}
		$payload = $this->callJwtStatic( 'decode', array( $token, $keys ) );
		if ( $audience ) {
		...
		$issuers = $issuer ? array( $issuer ) : array( self::OAUTH2_ISSUER, self::OAUTH2_ISSUER_HTTPS );
	 ...
	 */
	public function revoke( $token, array $options = array() ) {
		if ( \is_array( $token ) ) {
	 ...
	 * @throws InvalidArgumentException If received certs are in an invalid format.
	 */
	private function getCerts( $location, $cacheKey, array $options = array() ) {
	 ...
	 * @throws RuntimeException If certs could not be retrieved from a remote location.
	 */
	private function retrieveCertsFromLocation( $url, array $options = array() ) {
	...
	private function loadPhpsecPublicKey( string $modulus, string $exponent ): string {
		if ( \class_exists( \PostSMTP\Vendor\phpseclib\Crypt\RSA::class ) && \class_exists( \PostSMTP\Vendor\phpseclib\Math\BigInteger::class ) ) {
			$key = new \PostSMTP\Vendor\phpseclib\Crypt\RSA();
			$key->loadKey(
				array(
					'n' => new \PostSMTP\Vendor\phpseclib\Math\BigInteger( $this->callJwtStatic( 'urlsafeB64Decode', array( $modulus ) ), 256 ),
					'e' => new \PostSMTP\Vendor\phpseclib\Math\BigInteger( $this->callJwtStatic( 'urlsafeB64Decode', array( $exponent ) ), 256 ),
				)
			);
			return $key->getPublicKey();
		}
		$key = \PostSMTP\Vendor\phpseclib3\Crypt\PublicKeyLoader::load(
			array(
				'n' => new \PostSMTP\Vendor\phpseclib3\Math\BigInteger( $this->callJwtStatic( 'urlsafeB64Decode', array( $modulus ) ), 256 ),
				'e' => new \PostSMTP\Vendor\phpseclib3\Math\BigInteger( $this->callJwtStatic( 'urlsafeB64Decode', array( $exponent ) ), 256 ),
			)
		);
	...
	private function checkPhpsec3(): bool {
		return \class_exists( 'PostSMTP\\Vendor\\phpseclib3\\Crypt\\RSA' );
	}
	/**
	 * @return void
	 */
	private function checkSimpleJwt() {
		...
		if ( ! \class_exists( \PostSMTP\Vendor\SimpleJWT\JWT::class ) ) {
	 ...
	 * @return mixed
	 */
	protected function callJwtStatic( $method, array $args = array() ) {
		return \call_user_func_array( array( \PostSMTP\Vendor\Firebase\JWT\JWT::class, $method ), $args );
		// @phpstan-ignore-line
	}
	/**
	 * Provide a hook to mock calls to the JWT static methods.
	 ...
	 * @return mixed
	 */
	protected function callSimpleJwtDecode( array $args = array() ) {
		return \call_user_func_array( array( \PostSMTP\Vendor\SimpleJWT\JWT::class, 'decode' ), $args );
	}
	/**
