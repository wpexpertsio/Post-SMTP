 ...
 */
class OAuth2 implements \PostSMTP\Vendor\Google\Auth\FetchAuthTokenInterface {
	...
	public function generateCredentialsRequest() {
		...
		return new \PostSMTP\Vendor\GuzzleHttp\Psr7\Request( 'POST', $uri, $headers, \PostSMTP\Vendor\GuzzleHttp\Psr7\Query::build( $params ) );
	}
	/**
	...
	public function buildFullAuthorizationUri( array $config = array() ) {
		...
		$existingParams = \PostSMTP\Vendor\GuzzleHttp\Psr7\Query::parse( $result->getQuery() );
		$result         = $result->withQuery( \PostSMTP\Vendor\GuzzleHttp\Psr7\Query::build( \array_merge( $existingParams, $params ) ) );
	...
	private function getFirebaseJwtKeys( $publicKey, $allowedAlgs ) {
		...
		return array( new \PostSMTP\Vendor\Firebase\JWT\Key( $publicKey, $allowedAlg ) );
	}
	/**
