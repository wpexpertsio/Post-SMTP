...
use PostSMTP\Vendor\Google\Auth\GetQuotaProjectInterface;
...
use PostSMTP\Vendor\GuzzleHttp\Exception\ServerException;
...
class GCECredentials extends \PostSMTP\Vendor\Google\Auth\CredentialsLoader implements \PostSMTP\Vendor\Google\Auth\SignBlobInterface, \PostSMTP\Vendor\Google\Auth\ProjectIdProviderInterface, \PostSMTP\Vendor\Google\Auth\GetQuotaProjectInterface {
	 ...
	 */
	public function __construct( \PostSMTP\Vendor\Google\Auth\Iam $iam = null, $scope = null, $targetAudience = null, $quotaProject = null, $serviceAccountIdentity = null ) {
		$this->iam = $iam;
	...
	public static function onGce( callable $httpHandler = null ) {
		$httpHandler = $httpHandler ?: \PostSMTP\Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build( \PostSMTP\Vendor\Google\Auth\HttpHandler\HttpClientCache::getHttpClient() );
		...
		for ( $i = 1; $i <= self::MAX_COMPUTE_PING_TRIES; $i++ ) {
			try {
				...
				// the metadata resolution was particularly slow. The latter case is
				// "unlikely".
				$resp = $httpHandler( new \PostSMTP\Vendor\GuzzleHttp\Psr7\Request( 'GET', $checkUri, array( self::FLAVOR_HEADER => 'Google' ) ), array( 'timeout' => self::COMPUTE_PING_CONNECTION_TIMEOUT_S ) );
				return $resp->getHeaderLine( self::FLAVOR_HEADER ) == 'Google';
			} catch ( \PostSMTP\Vendor\GuzzleHttp\Exception\ClientException $e ) {
			} catch ( \PostSMTP\Vendor\GuzzleHttp\Exception\ServerException $e ) {
			} catch ( \PostSMTP\Vendor\GuzzleHttp\Exception\RequestException $e ) {
			} catch ( \PostSMTP\Vendor\GuzzleHttp\Exception\ConnectException $e ) {
			}
		}
		return \false;
	}
	...
	public function fetchAuthToken( callable $httpHandler = null ) {
		...
		if ( $this->targetAudience ) {
			return array( 'id_token' => $response );
		}
	...
	private function getFromMetadata( callable $httpHandler, $uri ) {
		$resp = $httpHandler( new \PostSMTP\Vendor\GuzzleHttp\Psr7\Request( 'GET', $uri, array( self::FLAVOR_HEADER => 'Google' ) ) );
