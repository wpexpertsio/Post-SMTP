...
if ( ! class_exists( 'PostmanViewController' ) ) {
	class PostmanViewController {
		...
		public function generateDefaultContent() {
			...
			add_action(
				'admin_print_styles-' . $mainPostmanSettingsPage,
				array(
					$this,
		...
		function enqueueHomeScreenStylesheet() {
			...
			wp_enqueue_script( self::POSTMAN_SCRIPT );
		}
		...
		public function addPurgeDataSubmenu() {
			$page = add_submenu_page(
				' ',
				sprintf( __( '%s Setup', 'post-smtp' ), __( 'Post SMTP', 'post-smtp' ) ),
				__( 'Post SMTP', 'post-smtp' ),
				Postman::MANAGE_POSTMAN_CAPABILITY_NAME,
				PostmanAdminController::MANAGE_OPTIONS_PAGE_SLUG,
				array(
					$this,
		...
		public function registerStylesAndScripts() {
			...
			wp_register_script(
				self::POSTMAN_SCRIPT,
				plugins_url( 'script/postman.js', $this->rootPluginFilenameAndPath ),
				array(
					self::JQUERY_SCRIPT,
					'jquery-ui-core',
			...
			wp_register_script(
				'jquery_steps_script',
				plugins_url( 'script/jquery-steps/jquery.steps.min.js', $this->rootPluginFilenameAndPath ),
				array(
					self::JQUERY_SCRIPT,
				),
				'1.1.0'
			...
			wp_register_script(
				'jquery_validation',
				plugins_url( 'script/jquery-validate/jquery.validate.min.js', $this->rootPluginFilenameAndPath ),
				array(
					self::JQUERY_SCRIPT,
				),
				'1.13.1'
					...
					<div class="ps-setting-box">
						...
						<div>
							<a href="<?php echo esc_url( 'https://postmansmtp.com/help-configure-post-smtp/' ); ?>" target="_blank" >
								<img src="<?php echo esc_url( POST_SMTP_ASSETS . 'images/icons/finger.png' ); ?>" width="15" />
								<?php echo esc_html__( 'Need help setup everything? (paid)', 'post-smtp' ); ?>
							</a>
						</div>
						<div>
						...
						if ( PostmanPreRequisitesCheck::isReady() ) {
							...
							if ( $this->options->isMailLoggingEnabled() ) {
								...
								printf(
									wp_kses_post(
										__(
											'The last %1$d email attempts are recorded <a href="%2$s">in the log</a>.',
											'post-smtp'
										)
									),
		...
		public function outputPurgeDataContent() {
			...
			print '<div class="wrap">';
			self::outputChildPageHeader( sprintf( '%s/%s/%s', $importTitle, $exportTile, $resetTitle ) );
			...
			printf(
				...
				esc_attr( $extraDeleteButtonAttributes ),
				esc_textarea( $data )
			);
			print '</section>';
