...
function portQuizTest(socket, hostname, port) {
	...
	var testEl       = jQuery( cell );
	testEl.html(
		'<span style="color:blue">' + postman_port_test.in_progress
		+ '</span>'
	);
	var data = {
					...
					if (postmanValidateAjaxResponseWithPopup( response )) {
						if (response.success) {
							...
							if (port == 443) {
								...
								addConclusion(
									sprintf(
										postman_port_test.try_dif_smtp,
										port,
										hostname
									),
									false,
...
function inspectResponse(id, response, port) {
	...
	if (response.reported_hostname_domain_only) {
		testEl.html(
			'<span>' + response.reported_hostname_domain_only
			+ '</span>'
		);
	}
