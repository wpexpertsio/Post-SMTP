...
if ( ! class_exists( 'PostmanDashboardWidgetController' ) ) {
    ...
    class PostmanDashboardWidgetController {
        ...
        public function __construct( $rootPluginFilenameAndPath, PostmanOptions $options, PostmanOAuthToken $authorizationToken, PostmanWpMailBinder $binder ) {
            ...
            add_action( 'admin_enqueue_scripts', array( $this, 'dasboardWidgetsScripts' ) );

            add_action( 'wp_dashboard_setup', array( $this, 'addDashboardWidget' ) );

            add_action( 'wp_network_dashboard_setup', array( $this, 'addNetworkDashboardWidget' ) );

            // dashboard glance mod
            if ( $this->options->isMailLoggingEnabled() ) {
                add_filter( 'dashboard_glance_items', array( $this, 'customizeAtAGlanceDashboardWidget' ), 10, 1 );
            }

            // Postman API: register the human-readable plugin state
            add_filter( 'print_postman_status', array( $this, 'print_postman_status' ) );
        }
        ...
        public function addDashboardWidget() {
            ...
            if ( PostmanUtils::isAdmin() ) {
                wp_add_dashboard_widget( 'example_dashboard_widget', __( 'Postman SMTP', 'post-smtp' ), array( $this, 'printDashboardWidget' ) ); // Display function.
                ...
                wp_add_dashboard_widget(
                    $widget_key,
                    __( 'Postman SMTP Stats', 'post-smtp' ),
                    array( $this, 'printStatsDashboardWidget' ),
                    'high'
                ); // Display function.
            }
        }
        ...
        public function addNetworkDashboardWidget() {
            // only display to the widget to administrator
            if ( PostmanUtils::isAdmin() ) {
                wp_add_dashboard_widget( 'example_dashboard_widget', __( 'Postman SMTP', 'post-smtp' ), array( $this, 'printNetworkDashboardWidget' ) ); // Display function.
            }
        }
        ...
        private function emailTypesSelectHtml() {
            ...
            // "Opened Emails" requires Pro + Reporting/Tracking extension (provides opened count).
            if ( post_smtp_has_pro() ) {
                $options['opened_emails'] = esc_html__( 'Opened Emails', 'post-smtp' );
            }

            // Disable non-"All Emails" choices when Pro isn't active.
            ...
            <select id="post-smtp-dash-widget-email-type" class="post-smtp-dash-widget-select-email-type" title="<?php esc_attr_e('Select email type', 'post-smtp'); ?>">
                ...
                <?php foreach ( $options as $key => $title ) : ?>
                    ...
                    // "Opened Emails" is available only when the Reporting and Tracking extension is active.
                    if ( 'opened_emails' === $key && ! class_exists( 'Post_SMTP_Report_And_Tracking' ) ) {
                        $option_disabled = 'disabled';
                    }
                    ?>
                    <option value="<?php echo sanitize_key( $key ); ?>" <?php echo $option_disabled; ?>>
        ...
        private function TimespanSelectHtml() {
            ...
            <select id="post-smtp-dash-widget-timespan" class="post-smtp-dash-widget-select-timespan" title="<?php esc_attr_e( 'Select timespan', 'post-smtp' ); ?>">
                ...
                foreach ( array( 7, 14, 30 ) as $option ) :
                    ?>
                    ...
                    <div class="post-smtp-dash-widget-settings-menu--style">
                        ...
                        <div>
                            <div class="post-smtp-dash-widget-settings-menu-item">
                                <input type="radio" id="post-smtp-dash-widget-settings-style-bar" class="post-smtp-dash-widget-settings-style" name="post-smtp-chart-style" value="bar" <?php echo 'bar' === $chart_style ? 'checked' : ''; ?> <?php echo $disabled; ?>>
                                <label for="post-smtp-dash-widget-settings-style-bar"><?php esc_html_e('Bar', 'post-smtp'); ?></label>
                            </div>
                            <div class="post-smtp-dash-widget-settings-menu-item">
                                <input type="radio" id="post-smtp-dash-widget-settings-style-line" class="post-smtp-dash-widget-settings-style" name="post-smtp-chart-style" value="line" <?php echo 'line' === $chart_style ? 'checked' : ''; ?> <?php echo $disabled; ?>>
        ...
        private function emailStatsBlock() {
            ...
            if ( post_smtp_has_pro() ) {
                $output_data['opened_emails'] = array(
                    ...
                    'title' => 'Opened ' . $opened_count, // Show the actual count of opened emails.
                );
            }
            ?>
        ...
        public function print_postman_status() {
            if ( ! PostmanPreRequisitesCheck::isReady() ) {
                ...
                if ( PostmanOptions::RUN_MODE_PRODUCTION !== $this->options->getRunMode() ) {
         ...
         * @return string
         */
        public function customizeAtAGlanceDashboardWidget( $items = array() ) {
