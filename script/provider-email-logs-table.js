        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success && response.data.logs && response.data.logs.length > 0) {
                    ...
                    providerLogs = [];
                    $('#ps-email-log').hide();
                    $('#ps-email-log tbody .ps-no-logs-row').show().text(postman_provider_logs.none_label || 'No logs found.');
                }
            },
            error: function() {
                $('#ps-email-log').hide();
                $('#ps-email-log tbody .ps-no-logs-row').show().text(postman_provider_logs.error_label || 'Error loading logs.');
            }
        });
    }
